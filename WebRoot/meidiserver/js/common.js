			...
			.bind(
					...
					function() {
						...
						if (pre_scrollTop != ($("#wrap").scrollTop() || document.body.scrollTop)) {
							...
							if (obj_th) {
								obj_th.style.top = ($("#wrap").scrollTop() || document.body.scrollTop)
										+ "px";
							}
...
function initOrder(type, statues, num, page, sort, sear) {
	...
	$.ajax({
		...
		data : "method=GETLIST&type=" + type + "&statues=" + statues + "&num="
				+ num + "&page=" + page + "&sort=" + sort + sear,
		dataType : "",
...
function save(method) {
	...
	$.ajax({
		type : "post",
		url : url + "server.jsp",
		data : "method=" + method + "&id=" + attract.toString() + "&branchid="
				+ branch.toString() + "&name=" + str + "&message=" + mess,
		dataType : "",
		success : function(data) {

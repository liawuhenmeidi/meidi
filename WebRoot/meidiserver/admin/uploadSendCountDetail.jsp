...
<%@ page language="java"
         import="java.util.*,wilson.upload.*,net.sf.json.JSONObject,uploadtotalgroup.*,utill.*,wilson.matchOrder.*,uploadtotal.*,user.*,wilson.salaryCalc.*;"
...
<table border="1px" align="left" width="100%">
        ...
        if (null != list) {
            for (int i = 0; i < list.size(); i++) {
                ...
                for (int j = 0; j < sendTypes.size(); j++) {
                    ...
                    if ((branch.equals(sain.getShop()) || StringUtill.isNull(branch)) && type.equals(StringUtill.getStringNocn(st.getType()))) {
                        String tpe = "";
                        if (null != mapus) {
                            UploadSalaryModel up = mapus.get(StringUtill.getStringNocn(st.getType()));
                            if (null != up) {
                                tpe = up.getCatergory();
                            }
                        }
    ...
    <tr class="asc" onclick="updateClass(this)">
        ...
        <td align="center"><%=sain.getType() %>
        </td>
        <td align="center"><%=DoubleUtill.getdoubleTwo(sain.getSalePrice())  %>
        </td>
        <td align="center"><%=sain.getNum() %>
        </td>
        <td align="center"><%=sain.getSalePrice() %>
        </td>
        <td align="center"><%=sain.getBackPoint() %>
        </td>
        <td align="center"><%=DoubleUtill.getdoubleTwo(sain.getSalePrice() * sain.getNum() * (1 - sain.getBackPoint() / 100) / sain.getNum()) %>
        </td>
        <td align="center"><%=DoubleUtill.getdoubleTwo(sain.getSalePrice() * sain.getNum() * (1 - sain.getBackPoint() / 100)) %>
        </td>
    </tr>
    <%
                    }

...
<%@ page language="java"
         import="order.Order,uploadtotal.UploadTotal,uploadtotalgroup.UploadTotalGroup,uploadtotalgroup.UploadTotalGroupManager,user.User,utill.BasicUtill,utill.DoubleUtill,utill.StringUtill,wilson.salaryCalc.*"
         pageEncoding="UTF-8" contentType="text/html;charset=utf-8" %>
<%@ page import="wilson.upload.SendType" %>
<%@ page import="wilson.upload.UploadManager" %>
<%@ page import="wilson.upload.UploadOrder" %>
<%@ page import="wilson.upload.UploadSalaryModel" %>
...
<form action="" method="post" id="post" onsubmit="return checkedd()">
    ...
    <select name="said" id="said">
        ...
        <%
            for (int i = 0; i < orderNames.size(); i++) {
                ...
                if (sain.equals(id)) {
        %>
        <option value="<%=sain %>" selected="selected"><%=sain %>
        </option>
        <%
        } else {
        %>
        <option value="<%=sain %>"><%=sain %>
        </option>
        <%
                }
            }
    ...
    <input type="submit" value="品类门店统计"
           onclick="$('#type').val('totalcategory');$('#realcheckedStatus').val('<%=checkedStatus%>')"/>
    <input type="submit" value="门店品类统计"
           onclick="$('#type').val('total');$('#realcheckedStatus').val('<%=checkedStatus%>')"/>
    <input type="submit" value="型号统计"
           onclick="$('#type').val('typetotal');$('#realcheckedStatus').val('<%=checkedStatus%>')"/>
    <input type="submit" class="noprint" value="导出"
           onclick="$('#method').val('<%=type %>');$('#post').attr('action','../Print?type=<%=BasicUtill.send%>')"/>
    ...
    <div id="wrapsearch"
         style="position:fixed;text-align:center; top:50%;background-color:white; left:30%; margin:-20% 0 0 -20%; height:50%; width:50%; z-index:999;display:none">
        <div id="tablesearch" style="display:none">
            <table cellspacing="1" style="margin:auto;background-color:black; width:80%;height:80%;">
                ...
                <tr class="bsc">
                    <td class="center"><input type="button" onclick="changeprintln()"
                                              style="background-color:#ACD6FF;font-size:25px;width:200px" value="确定"/>
                    </td>
                </tr>
        ...
        <div id="tablechecked" style="display:none">
            <table cellspacing="1" style="margin:auto;background-color:black; width:80%;height:80%;">
                ...
                <tr class="bsc">
                    <td class="center"><input type="button" onclick="changechecked()"
                                              style="background-color:#ACD6FF;font-size:25px;width:200px" value="确定"/>
                    </td>
                </tr>
...
<table border="1px" align="left" width="100%">
    <tr>
        ...
        <td align="center">pos序号</td>
        <td align="center">pos单号</td>
    ...
    <tr class="asc" onclick="updateClass(this)">
        ...
        <td align="center"><%=sain.getPosNo() %>
        </td>
        <td align="center"><%=sain.getShop() %>
        </td>
        <td align="center"><%=sain.getSaleTime() %>
        </td>
        <td align="center"><%=tpe %>
        </td>
        <td align="center"><%=st.getType() %>
        </td>
        <td align="center"><%=DoubleUtill.getdoubleTwo(st.getSalePrice())  %>
        </td>
        <td align="center"><%=st.getNum() %>
        </td>
        <td align="center"><%=sendStatues.toString() %>
        </td>
        <td align="center"><%=DoubleUtill.getdoubleTwo(st.getSalePrice() * st.getNum()) %>
        </td>
        <td align="center"><%=sain.getBackPoint() %>
        </td>
        <td align="center"><%=DoubleUtill.getdoubleTwo(st.getSalePrice() * (1 - sain.getBackPoint() / 100)) %>
        </td>
        <td align="center"><%=DoubleUtill.getdoubleTwo(st.getSalePrice() * st.getNum() * (1 - sain.getBackPoint() / 100)) %>
        </td>
    </tr>
    <%
                }
    ...
    <tr class="asc" style="background:#ff7575" onclick="updateClass(this)">
        ...
        <td align="center"></td>
        <td align="center"></td>
        <td align="center"><%=DoubleUtill.getdoubleTwo(bpmoneycount)%>
        </td>
    </tr>
...
<table border="1px" align="left" width="100%">
    ...
    <tr class="asc"
        ondblclick="detail('uploadSendCountDetail.jsp?branch=<%=up.getBranchname() %>&type=<%=up.getType() %>&said=<%=id %>&totaltype=<%=BasicUtill.send %>&checkedStatus=<%=checkedStatus %>')"
        onclick="updateClass(this)">
        <td align="center" class="noprinln1"><%=idcount %>
        </td>
        <td align="center" class="noprinln2"><%=up.getBranchname() %>
        </td>
        <td align="center" class="noprinln3"><%=tpe%>
        </td>
        <td align="center" class="noprinln4"><%=up.getType()%>
        </td>
        <td align="center"
            class="noprinln5"><%=0 == up.getCount() ? "" : DoubleUtill.getdoubleTwo(up.getTotalcount() / up.getCount()) %>
        </td>
        <td align="center" class="noprinln6"><%=up.getCount() %>
        </td>
        <td align="center" class="noprinln7"><%=DoubleUtill.getdoubleTwo(up.getTotalcount()) %>
        </td>
        <td align="center"
            class="noprinln8"><%=0 == up.getCount() ? "" : DoubleUtill.getdoubleTwo(up.getTatalbreakcount() / up.getCount()) %>
        </td>
        <td align="center" class="noprinln9"><%=DoubleUtill.getdoubleTwo(up.getTatalbreakcount()) %>
        </td>
    </tr>
    ...
    <tr class="asc" style="background:orange" onclick="updateClass(this)">
        ...
        <td align="center" class="noprinln2"><%=branchname %>
        </td>
        <td align="center" class="noprinln3"><%=key%>
        </td>
        <td align="center" class="noprinln4"></td>
        <td align="center" class="noprinln5"></td>
        <td align="center" class="noprinln6"><%=initCount %>
        </td>
        <td align="center" class="noprinln7"><%=DoubleUtill.getdoubleTwo(initTotalcount) %>
        </td>
        <td align="center" class="noprinln8"></td>
        <td align="center" class="noprinln9"><%=DoubleUtill.getdoubleTwo(initTatalbreakcount) %>
        </td>
    </tr>
    ...
    <tr class="asc" style="background:#ff7575" ondblclick="unconfire('<%=branchname%>')" onclick="updateClass(this)">
        ...
        <td align="center" class="noprinln4"></td>
        <td align="center" class="noprinln5"></td>
        <td align="center" class="noprinln6"><%=Count %>
        </td>
        <td align="center" class="noprinln7"><%=DoubleUtill.getdoubleTwo(Totalcount) %>
        </td>
        <td align="center" class="noprinln8"></td>
        <td align="center" class="noprinln9"><%=DoubleUtill.getdoubleTwo(Tatalbreakcount) %>
        </td>
    </tr>
    ...
    %>
    <tr class="asc"
        ondblclick="detail('uploadSendCountDetail.jsp?type=<%=up.getType() %>&said=<%=id %>&totaltype=<%=BasicUtill.send %>&checkedStatus=<%=checkedStatus %>')"
        onclick="updateClass(this)">
        <td align="center" class="noprinln1"><%=idcount %>
        </td>
        <td align="center" class="noprinln2"></td>
    ...
    <tr class="asc" style="background:#ff7575" onclick="updateClass(this)">
        ...
        <td align="center" class="noprinln4"></td>
        <td align="center" class="noprinln5"></td>
        <td align="center" class="noprinln6"><%=AllCount %>
        </td>
        <td align="center" class="noprinln7"><%=DoubleUtill.getdoubleTwo(AllTotalcount) %>
        </td>
        <td align="center" class="noprinln8"></td>
        <td align="center" class="noprinln9"><%=DoubleUtill.getdoubleTwo(AllTatalbreakcount) %>
        </td>
    </tr>
</table>
<%} %>

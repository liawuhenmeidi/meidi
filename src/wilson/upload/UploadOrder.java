...
public class UploadOrder {
    ...
    public List<SendType> getSendType() {
        ...
        if (StringUtill.isNull(this.saleManName)) {
            ...
            st.setNum(this.getNum());
            st.setSalePrice(this.getSalePrice());
            li.add(st);
        } else {
                for (int i = 0; i < saleManName.split(",").length; i++) {
                    int id = Integer.parseInt(saleManName.split(",")[i].split(":")[0]);
                    ...
                    SendType st = new SendType();
                    st.setType(ProductService.getIDmap().get(id).getType());
                    st.setNum(Integer.valueOf(saleManName.split(",")[i].split(":")[1]));
                    st.setSalePrice(Double.valueOf(saleManName.split(",")[i].split(":")[2]));
                    li.add(st);
                }
        }
        return li;
